...
def plot_best_15(specfile, overall_source, output):
    ...
    if(specfile==''):
        return(0, 0)
    z,z_err = [],[]
    input_spec = source+'/'+fname+'_snidflux.dat'
...
def parse_output(specfile, overall_source, returnoutput = False):
    fname = specfile.split("/")[-1].split(".")[0]
    source = overall_source + fname
    
    f = open(source + "/" + fname + "_snid.output", "r")
    
    lines = f.readlines()
    Types_summary = lines[38:67]
    
    Types_summary_file = open(source + "/" + fname + "_snid_types.readableoutput", "w")
    for i in Types_summary:
        Types_summary_file.write(i)
    Types_summary_file.close()
    if(returnoutput):
        Types_Summary_Table = Table.read(source + "/" + fname + "_snid_types.readableoutput", format = "ascii")
    
    Template_Listings = lines[69::]
    Template_Listings_file = open(source + "/" + fname + "_snid_templates.readableoutput", "w")
    for i in Template_Listings:
        Template_Listings_file.write(i)
    Template_Listings_file.close()
    if(returnoutput):
        Template_Listings_Table = Table.read(source + "/" + fname + "_snid_templates.readableoutput", format = "ascii")
...
def run_all(fname, flist, source, output):
    ...
    plot_best_15(fname, source, output)
    return(parse_output(fname, source))
